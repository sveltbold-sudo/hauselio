...
export default function DeliveryEstimate() {
  const [delivery, setDelivery] = useState<{ from: string; to: string; fromISO: string; toISO: string } | null>(null);
  ...
  return (
    <span className="text-xs text-[var(--color-text-muted)]" aria-busy={!delivery}>
      {delivery ? (
        <>
          <time dateTime={delivery.fromISO}>{delivery.from}</time> - <time dateTime={delivery.toISO}>{delivery.to}</time>
        </>
      ) : (
