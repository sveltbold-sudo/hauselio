  ...
  return (
    <div className="animate-fade-in-up">
      ...
      >
        ...
        {/* Zoom indicator */}
        <div className="absolute bottom-4 right-4 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300">
      ...
      {images.length > 1 && (
        <div className="relative">
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-thin" role="listbox" aria-label="Produktbilder" aria-orientation="horizontal" aria-activedescendant={`thumb-${activeImageIndex}`}>
            {images.map((img, i) => (
              <button
                key={i}
                id={`thumb-${i}`}
              ...
              >
                <Image
                  src={img}
                  alt={`${name} — Bild ${i + 1} von ${images.length}`}
                  width={80}
                  height={80}
                  sizes="80px"
