...
export default function MiniCart() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {/* Dropdown */}
      {isOpen && (
        <div ref={dropdownRef} className={`absolute right-0 top-full mt-2 w-[min(calc(100dvw-2rem),380px)] bg-white rounded-2xl shadow-[var(--shadow-2xl)] border border-[var(--color-border-light)] z-[55] origin-top-right flex flex-col max-h-[80dvh] ${prefersReduced ? "" : "animate-scale-in"}`} role="dialog" aria-modal="true" aria-label="Warenkorb-Übersicht">
          {/* Header */}
          ...
          {items.length === 0 ? (
            ...
            <>
              ...
              <div className="p-5 border-t border-[var(--color-border-light)] shrink-0">
                ...
                {couponDiscount > 0 && (
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm text-[var(--color-success)]">Rabatt ({coupon?.label})</span>
                    <span className="text-sm font-medium text-[var(--color-success)]">-{formatPrice(couponDiscount)}</span>
                  </div>
                )}
                <div className="flex items-center justify-between mb-2">
