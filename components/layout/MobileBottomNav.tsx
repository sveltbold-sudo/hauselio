        ...
        {navItems.map((item) => {
          ...
          return (
            <li key={item.href}>
              <Link
                href={item.href}
                className={`flex flex-col items-center justify-center min-w-[52px] min-h-[44px] gap-0.5 text-[10px] transition-colors ${
                  isActive
              ...
              >
                ...
                {item.href === "/warenkorb" && itemCount > 0 && (
                  <span className="absolute -top-1 right-1 w-4 h-4 bg-[var(--color-accent)] text-white text-[9px] font-bold rounded-full flex items-center justify-center">
